...
impl TsifyContainerAttrs {
    ...
    pub fn from_nested_meta(
        ...
        if meta.path.is_ident("large_number_types_as_bigints") {
            if self.ty_config.large_number_types_as_bigints {
                return Err(meta.error("duplicate attribute"));
            }
            if cfg!(not(feature = "js")) {
                return Err(
                    meta.error("#[tsify(large_number_types_as_bigints)] requires the `js` feature")
                );
            }
            self.ty_config.large_number_types_as_bigints = true;
            return Ok(());
        }
